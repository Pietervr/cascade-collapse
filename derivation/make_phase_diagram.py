...
(A) Response curves x(l0) from Eq. (9), x = l0 + alpha*x*exp(-(1-x)*theta),
...
(B) The (alpha, l0) phase diagram at theta=1, computed (not schematic):
    collapse spinodal l0^c(alpha) [Eq. (10)], recovery spinodal
...
def panel_b(ax) -> None:
    ...
    ax.plot(al, l0c, "-", color=RED, lw=1.8,
            label=r"collapse spinodal $\ell_0^{c}(\alpha)$, Eq. (10)")
